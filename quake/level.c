...
# endif
  ...
  //@ First sector is reserved for the "void"
  {
...
# endif
  ...
  //@ First sidedef is reserved for the back of all one-sided
  //@ linedefs --> the Void space.
  {
    sidedef_t *side = NewSidedef();

    side->sector = LookupSector(0);
    side->sector->ref_count++;

    strcpy(side->upper_tex, "void");
    strcpy(side->lower_tex, "void");
    strcpy(side->mid_tex,   "void");
  ...
  for (i=0; i < count; i++, raw++)
  {
    ...
    side->sector = (SINT16(raw->sector) == -1) ? LookupSector(0) :
        LookupSector(UINT16(raw->sector) + 1);
...
{
  if (num == 0xFFFF)
    return LookupSidedef(0);

  if ((int)num >= num_sidedefs && (sint16_g)(num) < 0)
    return LookupSidedef(0);

  return LookupSidedef(num + 1);
}
